...
static int __write_header(FILE* log_fd)
{
    ...
    {
        fprintf(log_fd, ",rm3100_x,rm3100_y,rm3100_z,rm3100_time_ns");
    }
...
static int __write_log_entry(FILE* log_fd, log_entry_t e)
{
    ...
    if (settings.log_rm3100)
    {
        fprintf(log_fd, ",%.4lf,%.4lf,%.4lf,%" PRIu64,
                 e.rm3100_x, e.rm3100_y, e.rm3100_z, e.rm3100_time_ns);
    }
    if(settings.log_benchmark){
...
//    e.xbee_yaw = xbeeMsg.yaw;
    ...
    e.rm3100_x = rm3100_data.mag[0];
    e.rm3100_y = rm3100_data.mag[1];
    e.rm3100_z = rm3100_data.mag[2];
    e.rm3100_time_ns = rm3100_data.timestamp;
