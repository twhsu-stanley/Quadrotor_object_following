...
static int __parse_thrust_map(void)
{
    ...
    else if (strcmp(tmp_str, "QUADLAB_MAP") == 0)
    {
        settings.thrust_map = QUADLAB_MAP;
    }
    else
    {
...
static int __parse_flight_mode(json_object* jobj_str, flight_mode_t* mode)
{
    ...
    else if (strcmp(tmp_str, "TAKE_OFF") == 0)
    {
        *mode = TAKE_OFF;
    }
    else if (strcmp(tmp_str, "LAND") == 0)
    {
        *mode = LAND;
    }
...
int settings_load_from_file(const char* path)
{
    ...
    PARSE_CONTROLLER(yaw_visual_controller)
