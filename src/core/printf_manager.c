...
static int __print_header()
{
    ...
    {
        printf("%s x_odo | y_odo | z_odo | dYaw | dist | objlok |", __next_colour());
    }
    ...
    while (rc_get_state() != EXITING)
    {
        ...
        if (settings.printf_followme)
        {
            printf("%s%+6.2f|%+6.2f|%+6.2f|%+6.2f|%+6.2f|%+2d|", __next_colour(),
                visual_odometry.x,visual_odometry.y,visual_odometry.z,
                state_estimate.visual_bearing, state_estimate.visual_range, state_estimate.object_tracking);
        ...
        if (settings.printf_rm3100)
        {
            printf("%s%+8.3lf|%+8.3lf|%+8.3lf|%+8.3lf|", __next_colour(),
                    rm3100_data.mag[0], rm3100_data.mag[1], rm3100_data.mag[2],
                    sqrt(pow(rm3100_data.mag[0], 2) + pow(rm3100_data.mag[1], 2) + pow(rm3100_data.mag[2], 2) )
                  );
        }
        if (settings.printf_motors)
        {
...
int print_flight_mode(flight_mode_t mode)
{
    switch (mode)
    {
        ...
        case ALT_HOLD:
            ...
            return 0;
        case TAKE_OFF:
            printf("%sTAKE_OFF         %s", KBLU, KNRM);
            return 0;
        case LAND:
            printf("%sLAND         %s", KBLU, KNRM);
            return 0;
        case FOLLOW_ME:
